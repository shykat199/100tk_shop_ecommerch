<div class="product-card v2">
    <div class="product-img">
        <a href="{{ route('product', $product->slug) }}">
            @if ($product->images->first()->image ?? false)
                <img src="{{ asset('uploads/products/galleries/' . $product->images->first()->image ?? '') }}"
                    class="b-1" alt="{{ $product->name }}">
            @endif
        ...
        <ul class="product-cart">
            <li><a href="javascript:addToWishlist({{ $product->id }})"><span class="icon"><svg
            ...
            <li><a href="javascript:addToCart({{ $product->id }})"><span class="icon"><svg
    ...
    <div class="product-card-details">
        ...
        @if (hasPromotion($product->id))
            ...
            <span class=""><del class="text-secondary">{{ currency($product->unit_price) }}</del> <small
                    class="text-secondary">
                    {{ __('- ') }}{{ round((($product->unit_price - promotionPrice($product->id)) / $product->unit_price) * 100) }}{{ __('%') }}</small></span>
        @else
