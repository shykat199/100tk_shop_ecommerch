<?php

namespace App\Http\Controllers\Backend;

use App\Http\Controllers\Controller;
use App\Models\Backend\Campaign;
use App\Models\Backend\CampaignReview;
use App\Models\Backend\Product;
use App\User;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Storage;
use Illuminate\Support\Facades\URL;
use Illuminate\Support\Str;
use App\Http\Traits\ResponseMessage;
use Illuminate\Support\Facades\File;
use Intervention\Image\Image;

class CampaignController extends Controller
{
    use ResponseMessage;

    public function index(Request $request)
    {
        $show_data = Campaign::orderBy('id','DESC')->get();
        return view('backend.pages.campaign.index',compact('show_data'));
    }

    public function campaignList(Request $request)
    {
        $draw = $request->get('draw');
        $start = $request->get("start");
        $rowperpage = $request->get("length"); // total number of rows per page

        $columnIndex_arr = $request->get('order');
        $columnName_arr = $request->get('columns');
        $order_arr = $request->get('order');
        $search_arr = $request->get('search');

        $columnIndex = $columnIndex_arr[0]['column']; // Column index
        $columnName = $columnName_arr[$columnIndex]['data']; // Column name
        $columnSortOrder = $order_arr[0]['dir']; // asc or desc
        $searchValue = $search_arr['value']; // Search value

        $query = Campaign::query();
        // Total records
        $totalRecords = $query->count();
        $totalRecordswithFilter = $totalRecords;
        // Get records, also we have included search filter as well
        if (!empty($searchValue)) {
            $query->where(function ($qry) use ($searchValue) {
                    $qry->orWhere('name', 'like', '%' . $searchValue . '%');
                    $qry->orWhere('banner_title', 'like', '%' . $searchValue . '%');
                });
            $totalRecordswithFilter = $query->count();
        }
        $records = $query
            ->orderBy($columnName, $columnSortOrder)
            ->skip($start)
            ->take($rowperpage)
            ->get();

        $data_arr = array();

        foreach ($records as $key => $record) {
            $class = 'warning';
            $lavelName = 'Active';
            if ($record->status == 1) {
                $class = 'success';
                $lavelName = 'Active';
            } elseif ($record->status == 0) {
                $class = 'danger';
                $lavelName = 'InActive';
            }

            $data_arr[] = array(
                "id" => $record->id,
                "name" => $record->name,
                "banner_title" => $record->banner_title,
                "status" => '<div class="badge bg-'.$class.'">'.$lavelName.'</div>',
                "action" => '<ul>
                                <li>
                                        <a class="p-0 action" href="' . route('campaign',$record->slug) . '">
                                            <button title="View">
                                                <i class="fa fa-eye"></i>
                                            </button>
                                        </a>
                                </li>
                                <li>
                                        <a class="p-0 action" href="' . route('backend.campaign.edit',$record->id) . '">
                                            <button title="Edit">
                                                <svg viewBox="0 0 11 11" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M8.72031 5.31576C8.48521 5.31576 8.29519 5.50625 8.29519 5.74089V9.1421C8.29519 9.37634 8.1047 9.56722 7.87007 9.56722H1.91801C1.68331 9.56722 1.49289 9.37634 1.49289 9.1421V3.19C1.49289 2.95575 1.68331 2.76487 1.91801 2.76487H5.3192C5.5543 2.76487 5.74432 2.57438 5.74432 2.33975C5.74432 2.10504 5.5543 1.91455 5.3192 1.91455H1.91801C1.21483 1.91455 0.642578 2.4868 0.642578 3.19V9.1421C0.642578 9.84529 1.21483 10.4175 1.91801 10.4175H7.87007C8.57326 10.4175 9.14551 9.84529 9.14551 9.1421V5.74089C9.14551 5.50579 8.95541 5.31576 8.72031 5.31576Z"/>
                                                    <path d="M4.62759 4.9274C4.59785 4.95714 4.57785 4.99497 4.56936 5.03577L4.26879 6.53916C4.25477 6.60884 4.27688 6.68069 4.32702 6.73129C4.36742 6.77169 4.42184 6.79333 4.47758 6.79333C4.49112 6.79333 4.50521 6.79209 4.51923 6.78913L6.02218 6.48856C6.06383 6.48 6.10167 6.46007 6.13101 6.43025L9.49487 3.06645L7.99192 1.5636L4.62759 4.9274Z"/>
                                                    <path d="M10.5329 0.525254C10.1184 0.110723 9.444 0.110723 9.02982 0.525254L8.44141 1.11362L9.94444 2.61652L10.5329 2.02808C10.7336 1.82786 10.8441 1.56084 10.8441 1.27686C10.8441 0.992876 10.7336 0.725864 10.5329 0.525254Z"/>
                                                </svg>
                                            </button>
                                        </a>
                                </li>
                                <li>
                                             <form user="deleteForm" method="POST" action="' . route('backend.campaign.destroy', $record->id) . '">
                                                    ' . csrf_field() . method_field("DELETE") . '
                                                    <a class="p-0 action" href="javascript:void(0);"
                                                       onclick="deleteWithSweetAlert(event,parentNode);">
                                                        <button title="Delete">
                                                            <svg viewBox="0 0 10 11" xmlns="http://www.w3.org/2000/svg">
                                                                <path d="M8.65184 10.2545C8.63809 10.5288 8.36791 10.7452 8.03934 10.7452H2.31625C1.98768 10.7452 1.7175 10.5288 1.70375 10.2545L1.29504 3.04834H9.06055L8.65184 10.2545ZM3.88317 4.83823C3.88317 4.7234 3.77169 4.63027 3.63416 4.63027H3.23589C3.09844 4.63027 2.98688 4.72338 2.98688 4.83823V8.95529C2.98688 9.07014 3.09835 9.16324 3.23589 9.16324H3.63416C3.77163 9.16324 3.88317 9.07019 3.88317 8.95529V4.83823ZM5.62591 4.83823C5.62591 4.7234 5.51444 4.63027 5.37693 4.63027H4.97866C4.84121 4.63027 4.72968 4.72338 4.72968 4.83823V8.95529C4.72968 9.07014 4.84112 9.16324 4.97866 9.16324H5.37693C5.51441 9.16324 5.62591 9.07019 5.62591 8.95529V4.83823ZM7.36871 4.83823C7.36871 4.7234 7.25724 4.63027 7.11973 4.63027H6.72143C6.58396 4.63027 6.47245 4.72338 6.47245 4.83823V8.95529C6.47245 9.07014 6.58393 9.16324 6.72143 9.16324H7.11973C7.25721 9.16324 7.36871 9.07019 7.36871 8.95529V4.83823Z"/>
                                                                <path d="M1.0213 1.09395H3.66155V0.677051C3.66155 0.617846 3.71902 0.569824 3.78994 0.569824H6.56248C6.63337 0.569824 6.69083 0.617846 6.69083 0.677051V1.09393H9.33112C9.5436 1.09393 9.71582 1.23779 9.71582 1.41526V2.42468H0.636597V1.41528C0.636597 1.23782 0.808817 1.09395 1.0213 1.09395Z"/>
                                                            </svg>
                                                        </button>
                                                    </a>
                                                </form>

                                            </li>
                                        </ul>'
            );
        }

        $response = array(
            "draw" => intval($draw),
            "iTotalRecords" => $totalRecords,
            "iTotalDisplayRecords" => $totalRecordswithFilter,
            "aaData" => $data_arr,
        );

        return json_encode($response);
    }

    public function create()
    {
        $products = Product::select('id','name')->get();
        return view('backend.pages.campaign.create',compact('products'));
    }

    public function store(Request $request)
    {
        $this->validate($request, [
            'name' => 'required',
            'short_description' => 'nullable',
            'description' => 'nullable',
            'banner' => 'nullable|image|mimes:jpeg,png,jpg,webp|max:2048',
            'image_one' => 'nullable|image|mimes:jpeg,png,jpg,webp|max:2048',
            'image_two' => 'nullable|image|mimes:jpeg,png,jpg,webp|max:2048',
            'image_three' => 'nullable|image|mimes:jpeg,png,jpg,webp|max:2048',
            'product_id' => 'required|array|min:1|exists:products,id',
            'image.*' => 'nullable|image|mimes:jpeg,png,jpg,webp|max:2048',
            'review' => 'required',
            'deadline' => 'nullable|date|after:now', // Ensure deadline is a future date
            'top_title_1' => 'nullable|string|max:255',
            'top_title_2' => 'nullable|string|max:255',
            'heading_1' => 'nullable|string|max:255',
            'feature_1' => 'nullable|string|max:255',
            'feature_2' => 'nullable|string|max:255',
            'heading_2' => 'nullable|string|max:255',
            'heading_3' => 'nullable|string|max:255',
            'heading_4' => 'nullable|string|max:255',
            'note' => 'nullable|string|max:255',
            'billing_details' => 'nullable|string|max:255',

        ]);

        // Prepare the input data
        $input = $request->except('image', 'product_id');
        $input['status'] = true; // Set status to true if not checked

        // Handle the first selected product ID
        $firstProductId = $request->product_id[0];
        $input['product_id'] = $firstProductId;

        $singleImages = [
            'banner',
            'image_one',
            'image_two',
            'image_three'
        ];

        foreach ($singleImages as $field) {
            if ($request->hasFile($field)) {
                $file = $request->file($field);

                // Store inside storage/app/campaign/
                $path = Storage::putFile('campaign', $file);

                // Remove "campaign/" from beginning? (NO – you said store FULL path except "storage/")
                // So we keep "campaign/filename.ext"
                $input[$field] = $path;
            }
        }

        // Create slug
        $input['slug'] = strtolower(Str::slug($request->name));
        $input['video'] = $this->getYouTubeVideoId($request->video);

        // Create a new campaign
        $campaign = Campaign::create($input);
        // Attach remaining selected products to the pivot table
        $remainingProductIds = array_slice($request->product_id, 1);
        if (!empty($remainingProductIds)) {
            $campaign->products()->attach($remainingProductIds);
        }

        // Handle additional images (review images)
        if ($request->hasFile('image')) {
            foreach ($request->file('image') as $file) {

                // Store inside storage/app/campaign/
                $path = Storage::putFile('campaign', $file);

                // Save in DB as: campaign/xxxxxx.jpg
                $pimage = new CampaignReview();
                $pimage->campaign_id = $campaign->id;
                $pimage->image = $path;
                $pimage->save();
            }
        }

        return redirect()->route('backend.campaign.index')->with($this->create_success_message);
    }

    public function edit($id)
    {
        // Fetch the campaign with its related images and products
        $edit_data = Campaign::with('images')->findOrFail($id);


        $select_products = DB::select('
            SELECT products.id, products.name
            FROM products
            INNER JOIN campaign_products ON products.id = campaign_products.product_id
            WHERE campaign_products.campaign_id = ?
        ', [$id]);


        // Fetch all available products
        $products = Product::select('id', 'name')->get();

        return view('backend.pages.campaign.edit', compact('edit_data', 'products','select_products'));
    }

    public function update(Request $request)
    {
        $this->validate($request, [
            'name' => 'required',
            'short_description' => 'nullable',
            'description' => 'nullable',
            'banner' => 'nullable|image|mimes:jpeg,png,jpg,webp|max:2048',
            'image_one' => 'nullable|image|mimes:jpeg,png,jpg,webp|max:2048',
            'image_two' => 'nullable|image|mimes:jpeg,png,jpg,webp|max:2048',
            'image_three' => 'nullable|image|mimes:jpeg,png,jpg,webp|max:2048',
            'product_id' => 'required|array|min:1|exists:products,id',
            'image.*' => 'nullable|image|mimes:jpeg,png,jpg,webp|max:2048',
            'review' => 'required',
            'deadline' => 'nullable|date|after:now', // Ensure deadline is a future date
            'top_title_1' => 'nullable|string|max:255',
            'top_title_2' => 'nullable|string|max:255',
            'heading_1' => 'nullable|string|max:255',
            'feature_1' => 'nullable|string|max:255',
            'feature_2' => 'nullable|string|max:255',
            'heading_2' => 'nullable|string|max:255',
            'heading_3' => 'nullable|string|max:255',
            'heading_4' => 'nullable|string|max:255',
            'note' => 'nullable|string|max:255',
            'billing_details' => 'nullable|string|max:255',
        ]);
        // image one
        $update_data = Campaign::find($request->hidden_id);
        $input = $request->except('hidden_id','product_ids','files','image');
        $input['status'] = $request->has('status') ? 1 : 0;
        $input['video'] = $this->getYouTubeVideoId($request->video);
        $input['product_id'] = $request->product_id[0];

        $folderPath = public_path('uploads/campaign/');
        $dbFolder = "campaign/"; // THIS is stored in DB only

        if (!is_dir($folderPath)) {
            mkdir($folderPath, 0777, true);
        }

        // Handle Banner Image
        if ($request->hasFile('banner')) {
            $file = $request->file('banner');

            $name = time() . '-' . strtolower(str_replace(' ', '-', $file->getClientOriginalName()));
            $savePath = $folderPath . $name;            // real storage path
            $dbPath = $dbFolder . $name;                // stored in DB

            $file->move($folderPath, $name);

            // delete old file
            if ($update_data->banner && File::exists(public_path('uploads/' . $update_data->banner))) {
                File::delete(public_path('uploads/' . $update_data->banner));
            }

            $input['banner'] = $dbPath;  // only "campaign/filename.jpg"
        } else {
            $input['banner'] = $update_data->banner;
        }

        if ($request->hasFile('image_one')) {
            $file = $request->file('image_one');

            $name = time() . "-image-one.webp";
            $savePath = $folderPath . $name;
            $dbPath = $dbFolder . $name;

            $this->convertToWebp($file, $savePath);

            if ($update_data->image_one && File::exists(public_path('uploads/' . $update_data->image_one))) {
                File::delete(public_path('uploads/' . $update_data->image_one));
            }

            $input['image_one'] = $dbPath;
        } else {
            $input['image_one'] = $update_data->image_one;
        }
        // image two
        if ($request->hasFile('image_two')) {
            $file = $request->file('image_two');

            $name = time() . "-image-two.webp";
            $savePath = $folderPath . $name;
            $dbPath = $dbFolder . $name;

            $this->convertToWebp($file, $savePath);

            if ($update_data->image_two && File::exists(public_path('uploads/' . $update_data->image_two))) {
                File::delete(public_path('uploads/' . $update_data->image_two));
            }

            $input['image_two'] = $dbPath;
        } else {
            $input['image_two'] = $update_data->image_two;
        }
        // image three
        if ($request->hasFile('image_three')) {
            $file = $request->file('image_three');

            $name = time() . "-image-three.webp";
            $savePath = $folderPath . $name;
            $dbPath = $dbFolder . $name;

            $this->convertToWebp($file, $savePath);

            if ($update_data->image_three && File::exists(public_path('uploads/' . $update_data->image_three))) {
                File::delete(public_path('uploads/' . $update_data->image_three));
            }

            $input['image_three'] = $dbPath;
        } else {
            $input['image_three'] = $update_data->image_three;
        }
        // image four
        $input['slug'] = strtolower(Str::slug($request->name));
        $input['video'] = $this->getYouTubeVideoId($request->video);
        $update_data = Campaign::find($request->hidden_id);
        $update_data->update($input);

        // Update product relationship
        // Store the first selected product ID
        $firstProductId = $request->product_id[0];
        $update_data->product_id = $firstProductId;

        // Sync remaining selected products to the pivot table
        $remainingProductIds = array_slice($request->product_id, 1);
        $update_data->products()->sync($remainingProductIds);

        if ($request->hasFile('image')) {
            foreach ($request->file('image') as $file) {

                $name = time() . '-' . strtolower(str_replace(' ', '-', $file->getClientOriginalName()));
                $savePath = $folderPath . $name;
                $dbPath = $dbFolder . $name;

                $file->move($folderPath, $name);

                CampaignReview::create([
                    'campaign_id' => $update_data->id,
                    'image'       => $dbPath  // only "campaign/filename"
                ]);
            }
        }

        return redirect()->route('backend.campaign.index')->with($this->update_success_message);
    }
    private function convertToWebp($file, $outputPath)
    {
        $extension = strtolower($file->getClientOriginalExtension());
        $imagePath = $file->getRealPath();

        if (in_array($extension, ['jpg', 'jpeg'])) {
            $source = imagecreatefromjpeg($imagePath);
        } elseif ($extension == 'png') {
            $source = imagecreatefrompng($imagePath);
            imagepalettetotruecolor($source);
        } else {
            return false; // only jpg/png
        }

        // Save as WebP
        imagewebp($source, $outputPath, 90);

        imagedestroy($source);

        return true;
    }

//    public function inactive(Request $request)
//    {
//        $inactive = Campaign::find($request->hidden_id);
//        $inactive->status = 0;
//        $inactive->save();
//        Toastr::success('Success','Data inactive successfully');
//        return redirect()->back();
//    }
//
//    public function active(Request $request)
//    {
//        $active = Campaign::find($request->hidden_id);
//        $active->status = 1;
//        $active->save();
//        Toastr::success('Success','Data active successfully');
//        return redirect()->back();
//    }
    public function destroy(Request $request)
    {

        $delete_data = Campaign::find($request->hidden_id);
        $delete_data->delete();

        $campaign = Product::whereNotNull('campaign_id')->get();
        foreach($campaign as $key=>$value){
            $product = Product::find($value->id);
            $product->campaign_id = null;
            $product->save();
        }

        return redirect()->back()->with($this->delete_success_message);
    }
    public function imgdestroy(Request $request)
    {
        $delete_data = CampaignReview::find($request->id);
        File::delete($delete_data->image);
        $delete_data->delete();

        return redirect()->back()->with($this->delete_success_message);
    }
    public function getYouTubeVideoId($input)
    {
        // Check if the input is a valid YouTube video ID (11 characters long)
        if (preg_match('/^[a-zA-Z0-9_-]{11}$/', $input)) {
            return $input; // Return the ID directly if it's valid
        }

        // Regular expression to match YouTube video URLs
        $pattern = '/(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:[^\/\n\s]+\/\S+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([a-zA-Z0-9_-]{11})/';

        // Execute the regex pattern
        preg_match($pattern, $input, $matches);

        // Check if a match was found and return the video ID or null
        return isset($matches[1]) ? $matches[1] : null;
    }

}
